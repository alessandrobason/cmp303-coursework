    ...
    collider.setCollisionResponse(
        [](b2Body *body, void *udata) {
            ...
            if(layer >= LAYER_1 && layer <= LAYER_4) {
                Player *player = (Player *)((DynamicBody *)body->GetUserData().pointer)->getUserData();
                player->onHit();
            }
        }
    );
    ...
    sprite.updateCallback(
        [](Sprite &spr, void *udata) {
            if(spr.cur_animation == 0) {
                spr.cur_animation = 1;
                gmap.removeCollider(((Explosion *)udata)->getCollider());
                return;
            }
            ((Explosion *)udata)->setDead(true);
            // gmap.removeObject((GameObject *)udata);
        }, 
        this
    );
    if(strength) {
